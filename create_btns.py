...
def parse_categories_opt_into(categoriesArray, category_type, page):
  ...
  for category in categoriesArray:
    arrayBtns.append([InlineKeyboardButton(category['title']+" "+ str(category['wholesale_cost']), callback_data='opt_into_'+ category_type +'_old_' + category['chanel'] + '_' + category_type)])
...
def btns_recommendations_get(offset = 0):
  ...
  for data in recommendations_filter:
    keyboard.append([InlineKeyboardButton(data['title'] +" "+ str(data['price_now']), callback_data='watch_chanel_' + str(data['id']))])
