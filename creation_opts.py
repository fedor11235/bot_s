    ...
    if mode == 'opt-retail-price':
        try:
            ...
            await update.message.reply_text(
                "Напишите текущую(оптовую) стоимость размещения. Разница с розничной должна быть не менее 10%:")
        except:
        ...
        try:
            ...
            if int(opt['retail_price']) * 0.9 < wholesale_cost:
                await update.message.reply_text(
                    "Неверная оптовая стоимость. Разница с розничной должна быть не менее 10%:")
                return
            data = {'wholesale_cost': wholesale_cost}
            opt_set(user_id, data)
            user_change_message_mod(user_id, 'opt-minimum-permissible-value')
            await update.message.reply_text(
                "Введите минимальное количество мест, необходимое для оформления опта(от 3 до 10):")
        except:
            await update.message.reply_text("вы ввели неверные данные, повторите ввод")
        return
        ...
        try:
            ...
            if opt_minimum >= 3 and opt_minimum <= 10:
                ...
                if tariff_plan == 'lite':
                    ...
                    maximum = '30'
                await update.message.reply_text(
                    "Введите максимальное допустимое количество мест в опте(до " + maximum + "):")
            else:
                await update.message.reply_text("вы ввели неверные данные, повторите ввод")
        except:
            await update.message.reply_text("вы ввели неверные данные, повторите ввод")
        ...
        try:
            ...
            if opt_maximum > maximum:
                await update.message.reply_text(
                    "Выв ввели число мест больше вашего лимита: " + str(maximum) + " введите повторно")
                return
            elif opt_maximum < int(mini):
                await update.message.reply_text(
                    "Выв ввели число мест меньше вашего лимита: " + str(mini) + " введите повторно")
                return
