...
from utils import is_serialized_object, get_recommendation_message
...
*Приглашено пользователей:* ''' + str(profile['invitedUsers']),
    ...
    async def handler_callback(self, update: Update, context):
        ...
        if is_serialized_object(query.data):
            # обработка выбранных слотов времени в подборке
            ...
            context.user_data['user_id'] = user_id
            # подборки -- Slon Business✨
            if context.user_data['category'] == 'req': #  'opt':
                print('подборки')
                await self.process_reqs(query, context)

            else:
                print('category is not opt')
                print(query_array)
        ...
        Готово! Вы забронировали следующие даты:\n\n''' + booking_date_parse +
                ...
                # !Опт
                    ...
                    if query_array[1] == 'confirm':
                        ...
                        print('one of BD categories')
                        start_cut = 1
                        finish_cut = 10
                        page = 1
                        # before: opt_into_all_init, now: opt-into_all_init
                        # now: category_type
                        # query_array[1]
        ...
        ''', reply_markup=reply_markup, parse_mode="Markdown")
                    ...
                    if query_array[1] == 'opt-into':
                        ...
                        if query_array[-2] == 'more':
                            ...
                            keyboard = [[InlineKeyboardButton("Я сделаю это позже",
                                                              callback_data='recommendation-creative-accept')]]
                            reply_markup = InlineKeyboardMarkup(keyboard)
                            await query.edit_message_text(
                                'Отправьте креативы, кнопки присылайте отдельным сообщением:\n\nВы можете отправить их позже в профиле.',
                                reply_markup=reply_markup)
    ...
    *Приглашено пользователей:* ''' + str(profile['invitedUsers']),
    ...
    # new method for req branch
    async def process_reqs(self, query, context):
        print('process_reqs')
        query_array = query.data.split('_')
        user_id = query.message.chat.id
        delete = 'enabled'
        if query_array[0] == 'chanel-open':
            # goto here
            # watch offer
            chosen_offer_name = query_array[1]
            recommendation = recommendations_ind_get(chosen_offer_name)
            context.user_data['username'] = recommendation['username']
            context.user_data['chosen_offer'] = chosen_offer_name
            keyboard = [
                [InlineKeyboardButton("Назад", callback_data='list-all')],
                [InlineKeyboardButton("Выбрать даты",
                                      callback_data='chanel-select-dates_' + recommendation['username'] + '_0')],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            booking_date = recommendation['data_list'].split('_')
            # booking_date_parse = parse_view_date(booking_date)
            ...
            if allowed_dates[0] == '':
                allowed_dates_len = 0
            text = get_recommendation_message(recommendation, allowed_dates_len)
            await query.edit_message_text(text, reply_markup=reply_markup, parse_mode="Markdown")
            return
        elif query_array[0] == 'chanel-select-dates':
            print('chanel-select-dates')
            username = ''
            username_array = query_array[1:-1]
            if len(username_array) > 0:
                username = '_'.join(username_array)
            else:
                username = query_array[0]
            ...
            if query_array[-2] == 'more':
                print('more dates')
                username_array = query_array[1:-2]
                if len(username_array) > 0:
                    username = '_'.join(username_array)
                else:
                    username = query_array[0]
            ...
            opt = set_opt_recommendation_into(user_id, username,
                                              {'booking_date': str_booked, 'allowed_dates': str_allowed},
                                              delete)
            ...
            await query.edit_message_text('Выберите доступные для брони слоты:', reply_markup=reply_markup)
            print('----------------------')
        elif query_array[0] == 'chanel-select-time':
            if query_array[-1] == 'time-confirm':
                # GGHHHHHH
                print('line 1096')
                opt_old = set_opt_into(user_id, query_array[1], {'status': 'confirm'}, 'none')
                set_any_profile(user_id, {'opt_into_temp': query_array[1]})
                ...
                await query.edit_message_text(
                    ...
                    reply_markup=reply_markup)
                return
        elif query_array[0] == 'list-all':
            print('line 983')
            profile = get_profile(user_id)
            if profile['tariffPlan'] == 'base' or profile['tariffPlan'] == 'lite' or profile[
                'tariffPlan'] == 'pro':
                keyboard = [
                    [InlineKeyboardButton("Lite — 290₽/мес.", callback_data='pay_lite')],
                    [InlineKeyboardButton("Pro — 890₽/мес.", callback_data='pay_pro')],
